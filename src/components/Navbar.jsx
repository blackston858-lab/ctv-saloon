    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                ...
                <span className={`relative z-20 ${isActive ? "text-white" : "text-[#1A1A1A] hover:text-[#1B4D43]"}`}>
                  {link.name}
                </span>
              </Link>
            );
        ...
        <div className="hidden md:block">
          <Link
            to="/booking"
            className="bg-[#1B4D43] hover:bg-[#133831] text-white text-xs lg:text-sm font-semibold tracking-wide px-6 py-2.5 rounded-full shadow-md hover:shadow-lg hover:shadow-[#1B4D43]/10 transition-all duration-300 active:scale-95 block"
          >
            Book Appointment
          </Link>
        </div>
              ...
              {navLinks.map((link) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`text-sm font-medium tracking-wide transition-colors duration-200 ${isActive ? "text-white font-semibold" : "text-[#1A1A1A]"}`}>
                      {link.name}
                    </span>
                  </Link>
                );
